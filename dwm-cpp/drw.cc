...
void
drw_rect(Drw *drw, int x, int y, uint32_t w, uint32_t h, int filled, int invert)
{
	if (!drw || !drw->scheme)
		return;
...
int
drw_text(Drw *drw, int x, int y, uint32_t w, uint32_t h, uint32_t lpad, const char *text, int invert)
{
	char buf[1024];
	int ty;
	uint32_t ew;
	...
	size_t i, len, utf8strlen, utf8charlen;
	int render = x || y || w || h;
...
void
drw_font_getexts(Fnt *font, const char *text, uint32_t len, uint32_t *w, uint32_t *h)
{
	XGlyphInfo ext;
