...
//#define INTERSECT(x,y,w,h,m)    (std::m((x)+(w),(m)->wx+(m)->ww) - std::max((size_t)(x),((m)->wx)) \
//                               * std::min((y)+(h),(m)->wy+(m)->wh) - std::max((size_t)(y),((m)->wy)))
#define INTERSECT(x, y, w, h, m) (std::max((size_t)0, std::min((size_t)((x) + (w)), (size_t)((m)->mx + (m)->mw)) - std::max((size_t)(x), (size_t)((m)->mx))) \
    * std::max((size_t)0, std::min((size_t)((y) + (h)), (size_t)((m)->my + (m)->mh)) - std::max((size_t)(y), (size_t)((m)->my))))
...
#define dbg(x) system(("echo " + std::to_string(x) + " >> debug.log").c_str())
...
static void autostart(std::string path, bool blocking);
...
static void mutevolume(const Arg* arg);
...
static void resizemouse_any(const Arg* arg);
...
static bool scratchpad_last_showed_is_killed(void);
...
static int wmclasscontains(Window win, const char* _class, const char* name);
...
#ifdef VERSION
#include "IPCClient.cc"
#include "ipc.cc"
#include "yajl_dumps.cc"
#endif
...
void applyrules(Client* c)
{
  ...
  for (uint32_t i = 0; i < LENGTH(rules); i++) {
    ...
    if ((!r->title || strstr(c->name, r->title))
      ...
      if (m)
        c->mon = m;
      if (r->shouldMaximize && r->isfloating)
        resize(c, selmon->wx, selmon->wy, selmon->ww - borderpx, selmon->wh - borderpx, true);
    }
  }
  if (ch.res_class)
    XFree(ch.res_class);
...
int applysizehints(Client* c, size_t* x, size_t* y, size_t* w, size_t* h, int interact)
{
  int baseismin;
  Monitor* m = c->mon;

  /* set minimum possible */
  *w = std::max(1, (int32_t)(*w));
  *h = std::max(1, (int32_t)(*h));
  if (interact) {
    if (*x > sw)
      *x = sw - WIDTH(c);
    if (*y > sh)
...
void autostart(std::string path, bool blocking)
{
  system(("sh " + path + (blocking ? "" : "&")).c_str());
}
...
void buttonpress(XEvent* e)
{
  ...
  if (ev->window == selmon->barwin) {
    ...
    do
      x += TEXTW(tags[i]);
    while ((uint32_t)(ev->x) >= x && ++i < LENGTH(tags));

    if (i < LENGTH(tags)) {
      click = ClkTagBar;
      arg.ui = 1 << i;
    } else if ((uint32_t)(ev->x) < x + blw)
      click = ClkLtSymbol;
    else if ((uint32_t)(ev->x) > selmon->ww - TEXTW(stext))
      click = ClkStatusText;
    else
      click = ClkWinTitle;
  ...
  for (uint32_t i = 0; i < LENGTH(buttons); i++)
...
void clientmessage(XEvent* e)
{
  ...
  if (cme->message_type == netatom[NetWMState]) {
    if ((uint64_t)(cme->data.l[1]) == netatom[NetWMFullscreen]
...
void configurenotify(XEvent* e)
{
  ...
  if (ev->window == root) {
    dirty = (sw != (size_t)(ev->width) || sh != ev->height);
    sw = ev->width;
    sh = ev->height;
    if (updategeom() || dirty) {
      ...
      for (Monitor* m = mons; m; m = m->next) {
        for (Client* c = m->clients; c; c = c->next)     //
          if (c->isfullscreen)                           // Этого не было в 6.1 ... Виден ресайз фуллскриновых окон.
...
Monitor*
dirtomon(int dir) //И зачем оно нужно?
{
  Monitor* m = NULL;
...
//		return;
//	size_t x, w, tw = 0;
//	int32_t boxs = drw->fonts->h / 9;    //Что за магия?
//	int32_t boxw = drw->fonts->h / 6 + 2;
  ...
  if (ev->events & EPOLLIN) {
    ...
    while (running && XPending(dpy)) {
      ...
      if (handler[ev.type]) {
        ...
        ipc_send_events(mons, (Monitor**)&lastselmon, selmon);
      }
    }
...
void manage(Window w, XWindowAttributes* wa)
{
  ...
  c->y = std::max(c->y, ((c->mon->by == c->mon->my) && (c->x + (c->w / 2) >= c->mon->wx) && (c->x + (c->w / 2) < c->mon->wx + c->mon->ww)) ? bh : c->mon->my);
  c->bw = borderpx;
...
void movemouse(const Arg* arg)
{
  ...
  do {
    ...
    case MotionNotify:
      ...
      if (std::abs((int32_t)(selmon->wx - nx)) < snap)
        nx = selmon->wx;
      else if (std::abs((int32_t)((selmon->wx + selmon->ww) - (nx + WIDTH(c)))) < snap)
        nx = selmon->wx + selmon->ww - WIDTH(c);
      if (std::abs((int32_t)(selmon->wy - ny)) < snap)
        ny = selmon->wy;
      else if (std::abs((int32_t)((selmon->wy + selmon->wh) - (ny + HEIGHT(c)))) < snap)
        ny = selmon->wy + selmon->wh - HEIGHT(c);
      if (!c->isfloating && selmon->lt[selmon->sellt]->arrange
          && (std::abs((int32_t)(nx - c->x)) > snap || std::abs((int32_t)(ny - c->y)) > snap))
        togglefloating(NULL);
...
{
  //system("echo fuck >~/somefile"); - working
  bool mute = (exec("pamixer --get-mute") == "true\n"); //always false. wrf?
  system(("echo " + exec("pamixer --get-mute") + " >~/somefile").c_str());
  system("pamixer -t");
  system(fmt::format("echo {0} | sudo tee /sys/class/leds/platform::mute/brightness", char(int(!mute)+48)).c_str());
  //exec(("echo \""+exec(fmt::format("echo {} | sudo tee /sys/class/leds/platform::mute/brightness", char(int(!mute)+48)).c_str())+"\" > outp.out").c_str());
}
...
void resize(Client* c, size_t x, size_t y, size_t w, size_t h, int interact)
{
...
void resizemouse_any(const Arg* arg)
{
  ...
  if (c->isfullscreen) /* no support resizing fullscreen windows by mouse */
    return;
  restack(selmon);
  ocx = c->x; //Бэкап текущих координат окна в переменные?
  ocy = c->y; //
  och = c->h;
  ocw = c->w;
  if (XGrabPointer(dpy, root, false, MOUSEMASK, GrabModeAsync, GrabModeAsync,
  ...
  do {
    ...
    case MotionNotify:
      ...
      if (c->mon->wx + nw >= selmon->wx && c->mon->wx + nw <= selmon->wx + selmon->ww
        ...
        if (!c->isfloating && selmon->lt[selmon->sellt]->arrange
            && (std::abs((int32_t)(nw - c->w)) > snap || std::abs((int32_t)(nh - c->h)) > snap))
          togglefloating(NULL);
      }
      if (!selmon->lt[selmon->sellt]->arrange || c->isfloating)
...
void resizemouse(const Arg* arg)
{
  ...
  if (XGrabPointer(dpy, root, false, MOUSEMASK, GrabModeAsync, GrabModeAsync,
      ...
      != GrabSuccess)
    return;
  XWarpPointer(dpy, None, c->win, 0, 0, 0, 0, c->w + c->bw - 1, c->h + c->bw - 1); // Эта шляпа перемещает курсор
  do {
    XMaskEvent(dpy, MOUSEMASK | ExposureMask | SubstructureRedirectMask, &ev);
    switch (ev.type) {
    case ConfigureRequest:
    ...
    case MotionNotify:
      ...
      size_t nw = std::max(ev.xmotion.x - ocx - 2 * c->bw + 1, size_t(1));
      size_t nh = std::max(ev.xmotion.y - ocy - 2 * c->bw + 1, size_t(1));
      if (c->mon->wx + nw >= selmon->wx && c->mon->wx + nw <= selmon->wx + selmon->ww
          && c->mon->wy + nh >= selmon->wy && c->mon->wy + nh <= selmon->wy + selmon->wh) {
        if (!c->isfloating && selmon->lt[selmon->sellt]->arrange
            && (std::abs((int32_t)(nw - c->w)) > snap || std::abs((int32_t)(nh - c->h)) > snap))
  ...
  while (running) {
    ...
    for (int i = 0; i < event_count; i++) {
      ...
      if (event_fd == dpy_fd) {
        ...
        if (ipc_handle_client_epoll_event(events + i, mons, (Monitor**)&lastselmon, selmon,
...
void scan(void)
{
  ...
  if (XQueryTree(dpy, root, &d1, &d2, &wins, &num)) {
    ...
    for (uint32_t i = 0; i < num; i++) { /* now the transients */
      if (!XGetWindowAttributes(dpy, wins[i], &wa))
        continue;
...
void slankdev_view_right(const Arg* arg)
{
...
void slankdev_view_left(const Arg* arg)
{
  uint32_t mon = (selmon->tagset[selmon->seltags] >> 1);
  selmon->seltags ^= 1; /* toggle sel tagset */
  if (mon & TAGMASK)
...
int wmclasscontains(Window win, const char* _class, const char* name)
{
  XClassHint ch = { NULL, NULL };
  int res = 1;
  ...
  if (XGetClassHint(dpy, win, &ch)) {
    if (ch.res_name && strstr(ch.res_name, name) == NULL)
      res = 0;
    if (ch.res_class && strstr(ch.res_class, _class) == NULL)
      res = 0;
  } else
    res = 0;
...
int main(int argc, char* argv[])
{
  ...
  //		die("dwm: cannot open display");
  checkotherwm();
  //  XrmInitialize(); //xrdb, not working
  //  loadxrdb();
  setup();
  //#ifdef __OpenBSD__
  ...
  //#endif /* __OpenBSD__ */
  scan();
  autostart("$HOME/.config/dwm/autostart.sh", false);
  run();
  if (restart)
    execvp(argv[0], argv);
  cleanup();
