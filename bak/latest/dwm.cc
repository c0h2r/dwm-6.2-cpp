...
#define INTERSECT(x,y,w,h,m)    (std::min((x)+(w),(m)->wx+(m)->ww) - std::max((size_t)(x),((m)->wx)) \
...
void
autostart(std::string path, bool blocking)
{
...
int
gettextprop(Window w, Atom atom, char *text, uint32_t size)
{
	char **list = NULL;
	int n;
	XTextProperty name;
